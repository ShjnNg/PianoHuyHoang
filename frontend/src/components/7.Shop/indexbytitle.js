...
export default function PostList({ text }) {
  ...
  {await axios.get(`${URLchange}/api/count/countproductbytitle/${text.match.params.title}`).
  then((response) => {
  ...
  const getproductss = async(page, limit) => {
    try {
      ...
      await axios.get(`${URLchange}/api/search/title/${text.match.params.title}/${page}/${limit}`)
        .then((res) => {
          setprod(res.data)
